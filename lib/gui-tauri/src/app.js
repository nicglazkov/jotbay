...
const SYNC_INTERVAL = 600; // seconds; matches the scheduler
...
function renderActivity() {
  ...
  host.innerHTML = groups
    .map((g, i) => {
      const e = g.first;
      const expandable = e.files.length > 0 || (state.settings.verbose && e.detail);
      ...
      if (open) {
        if (e.files.length) {
          body += `<div class="event-files">${e.files
            .map((f) => `<div>${esc(f)}</div>`)
            .join("")}</div>`;
        }
  ...
  try {
    ...
    if (r.skipped_locked) {
      ...
      if (r.conflicts.length) {
        parts.push(`${r.conflicts.length} conflict(s) — both versions kept`);
      }
