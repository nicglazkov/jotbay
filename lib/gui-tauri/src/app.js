...
function render() {
  ...
  // System notifications for the onset of a problem, once per condition
  // rather than once per poll: the watcher retries every 20 seconds, and a
  // toast per retry is the activity-spam bug relocated to the notification
  // tray. Keyed by condition; a healed condition leaving the set makes its
  // return news again.
  const problems = new Set();
  for (const n of s.nodes) {
    const h = nodeHealth(n, s.head);
    if (h === "error") problems.add(`error-${n.hostname}\u0000${n.hostname} can't sync\u0000${n.last_error || "It reported a failure."}`);
    if (h === "missing") problems.add(`missing-${n.hostname}\u0000${n.hostname} has gone quiet\u0000No word from it in over a day.`);
  }
  if (s.rebase_in_progress) problems.add(`conflicts\u0000Conflicts need attention\u0000${s.conflicts.length} file(s) changed in two places. Both versions are kept.`);
  for (const w of s.warnings || []) {
    if (w.severity === "blocked") problems.add(`blocked-${w.path}\u0000${w.path.split("/").pop()} can't sync\u0000${w.advice || "Over the size limit."}`);
  }
  ...
  state.notified = new Set([...problems].map((p) => p.split("\u0000")[0]));
...
async function openDir(path) {
  ...
  // A place, not a button. It was labelled "Deleted" and sat beside an action,
  // which reads as a filter. Recovering a note is somewhere you go, the way
  // Photos and Mail treat it, so it belongs at the end of the list looking
  // like what it is.
  //
  // Top level only: inside a folder it would read as that folder's deleted
  // notes, and it is the whole vault's. Shown even when the folder is empty,
  // because somebody who has just deleted the last note is exactly who wants
  // it.
  const deletedRow =
    path === ""
      ? `<div class="frow deleted-row" data-deleted="true">
           <span class="ficon">&#128465;</span>
           <span class="fname">Recently Deleted</span>
           <span class="fmeta">&rsaquo;</span>
         </div>`
      : "";
  ...
  $("files-list").innerHTML = rows + deletedRow;
  $("files-list").querySelectorAll(".frow").forEach((row) =>
    row.addEventListener("click", () => {
      if (row.dataset.deleted === "true") return showDeletedNotes();
...
async function showDeletedNotes() {
  ...
  sheet("Recently Deleted", "Still in the history, and recoverable", rows, null);
  $("sheet-body")
    .querySelectorAll(".vrestore")
...
// Ctrl/Cmd+K: land in the search field from anywhere, switching to the Files
// tab first if Activity is showing. One keystroke in; Esc is the keystroke out.
window.addEventListener("keydown", (e) => {
  if (SETTINGS_ONLY) return;
  if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === "k") {
    e.preventDefault();
    showTab("files");
    const field = $("files-search");
    field.focus();
    field.select();
  }
  if (e.key === "Escape" && document.activeElement === $("files-search")) {
    $("files-search").value = "";
    $("files-search").dispatchEvent(new Event("input"));
    $("files-search").blur();
  }
});


// --- two versions of a note ---------------------------------------------------
//
// The one mess the app makes on its own: sync keeps both sides of a
// simultaneous edit, and until now abandoned you with the pair. The row
// appears above the file list only while pairs exist; the sheet settles them.
// All three choices keep both texts in history, and the sheet says so, because
// the fear at this moment is losing work.

async function refreshConflicts() {
  if (SETTINGS_ONLY) return;
  let pairs = [];
  try {
    pairs = await invoke("conflict_pairs");
  } catch (e) {
    return;
  }
  state.conflictPairs = pairs;
  const row = $("conflicts-row");
  row.hidden = pairs.length === 0;
  if (pairs.length) {
    $("conflicts-count").textContent =
      pairs.length === 1 ? "1 note has two versions" : `${pairs.length} notes have two versions`;
  }
}

$("conflicts-row").addEventListener("click", () => {
  const pairs = state.conflictPairs || [];
  const rows = pairs
    .map((p, i) => {
      const when = p.at ? whenText(p.at) : "";
      const who = esc(p.machine || "another machine");
      if (p.identical) {
        return `<div class="vrow"><div>
            <div class="vwhen">${esc(p.original)}</div>
            <div class="vmeta">Both versions are identical, so nothing is at stake.</div>
          </div>
          <button class="link csettle" data-copy="${esc(p.copy)}" data-choice="keep_current">Tidy up</button>
        </div>`;
      }
      return `<div class="vrow"><div>
          <div class="vwhen">${esc(p.original)}</div>
          <div class="vmeta">Also edited on ${who}${when ? " · " + when : ""}</div>
          <div class="cchoices">
            <button class="link csettle" data-copy="${esc(p.copy)}" data-choice="keep_current">Keep this version</button>
            <button class="link csettle" data-copy="${esc(p.copy)}" data-choice="keep_copy">Use ${who}'s</button>
            <button class="link csettle" data-copy="${esc(p.copy)}" data-choice="keep_both">Keep both</button>
          </div>
        </div></div>`;
    })
    .join("");
  sheet(
    "Two versions",
    "The same note was edited in two places",
    rows || `<div class="empty"><div class="empty-title">All settled</div></div>`,
    pairs.length ? "Whichever you choose, both versions stay in this note's history." : null
  );
  $("sheet-body").querySelectorAll(".csettle").forEach((b) =>
    b.addEventListener("click", async () => {
      try {
        await invoke("settle_conflict", { copy: b.dataset.copy, choice: b.dataset.choice });
        $("sheet").close();
        toast("Settled. It will sync with the next change.");
